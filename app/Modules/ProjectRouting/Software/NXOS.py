...
import time
import ipaddress
from app.Modules.ProjectRouting.Database import DatabaseOps as DataBaseOps
...
class RoutingNexus:
    ...
    def __init__(self, netmiko_connection, db_obj, db, cursor):
        ...
        self.create_db = DataBaseOps.RoutingDatabase(db, cursor)
    ...
    def database(self) -> None:
        ...
        try:
            DataBaseOps.db_update_nexus(self.db, self.cursor, vdc=self.vdc, vrf=self.vrf, prefix=self.prefix, protocol=self.protocol,
                                        admin_distance=self.admin_dis[0], metric=", ".join(self.metric), nexthops=", ".join(self.next_hop), 
                                        interfaces=", ".join(self.interface), tag=None, age=", ".join(self.route_age))
        except IndexError:
            pass
